...
| 09 | 04 + 결측 2개 이상 세그먼트 전용 서브모델 라우팅 | 0.94988 | ❌ 기각(개선폭 0) |

> 이번 데이터에서는 **Feature/모델을 더 추가하는 것이 항상 도움이 되지 않았다** — 이미 raw feature + 최소한의 복구 피처만으로 트리 모델이 핵심 상호작용을 거의 다 학습했기 때문에(3절), 추가 피처나 다른 모델과의 앙상블, 세그먼트 특화 서브모델까지 전부 노이즈로 작용하거나 무의미했던 사례가 총 8건(05, 06, 07-V1~V4, 08, 09) 확인되었다. 최종 Feature Set/모델은 **04(baseline + 복구 피처 2개, LightGBM 단독)** 로 확정하였다.
...
# 12-4. 추가 실험 — 결측 2개 이상 겹친 행 전용 서브모델 (기각)

마지막 남은 아이디어: 핵심 3피처 중 2개 이상 결측인 행(16,519개, 2.39%)을 위한 전용 서브모델을 만들어, 글로벌 모델(04) 대신 이 구간에서만 서브모델 예측을 쓰는 라우팅 앙상블을 시도하였다.

```python
specialist_params = dict(
    objective="multiclass", num_class=3, n_estimators=200, learning_rate=0.05,
    num_leaves=7, max_depth=4, min_child_samples=80,   # 작은 표본에 맞춘 보수적 설정
    subsample=0.8, colsample_bytree=0.8, reg_alpha=1.0, reg_lambda=1.0,
)
# 이 구간 행(13,000여 개)만으로 5-fold 학습
```

| 방식 | 이 구간 balanced accuracy |
| --- | --- |
| **글로벌 모델(04)** | **0.7428** ← 최고 |
| 휴리스틱(결측 패턴별 train 내 최빈 클래스) | 0.3333 |
| 전용 서브모델 | 0.7353 |

| | 전체 CV balanced accuracy |
| --- | --- |
| 글로벌 모델(04) | 0.94988 |
| 라우팅 앙상블 | 0.94988 |
| 개선폭 | **+0.00000** |

- **휴리스틱이 0.3333으로 완전히 무너짐**: 이 구간 안에서도 at-risk가 85.7%로 여전히 압도적이라, 패턴별 최빈클래스가 결국 거의 항상 at-risk만 찍는 "한 클래스만 찍기" 함정(3절 표의 방식 A와 동일한 실패)에 그대로 걸렸다.
- **전용 서브모델이 글로벌 모델보다 낮음**: 13,000여 개 표본으로는, 69만 행 전체로 학습해서 이 구간에도 남아있는 나머지 정보(예: `physical_activity_level`만 결측이 아닌 패턴이라면 그 피처의 강한 신호)를 이미 충분히 학습한 글로벌 모델을 이기지 못했다. **표본 크기가 지배적인 상황에서는 세그먼트 특화(specialization)가 오히려 손해**라는 것을 보여준 사례.

→ 라우팅 앙상블에서 최고 성능 방식이 글로벌 모델 자신으로 나왔으므로 전체 CV는 정확히 동일. **최종 제출은 `submission_v2_tuned.csv`(0.94987)를 계속 유지.** 이것으로 총 7가지 개선 아이디어가 전부 기각되어, 04의 결과를 이 프로젝트의 최종 결과로 확정하였다.
...
Feature Ablation(5절) 결과, 원본 피처 + 최소한의 결측 복구 피처 2개만으로 이미 이론적 상한(0.9412) 근처에 도달했으며, 여기에 추가 피처나 모델을 더하는 시도는 총 8가지(`stress_level` 예측 대치, `sleep_duration` 정밀 회귀 대치, interaction 피처, missing_count, native NaN, 이들의 결합, FT-Transformer 앙상블, 결측 세그먼트 전용 서브모델) 모두 실패하였다. 완전히 다른 모델 계열(FT-Transformer)과의 앙상블(12-3절)조차 노이즈 수준(+0.00007)의 이득에 그쳤다 — Transformer 단독 성능(0.94753)은 예상보다 훨씬 좋았지만, 라벨이 소수 피처의 명확한 규칙으로 생성돼 있어 LightGBM과 결국 같은 정답 함수를 근사하게 되고, 그만큼 두 모델의 오답 패턴도 겹쳐서 앙상블 다양성 효과가 거의 없었다. 가장 취약한 구간(결측 2개 이상, 12-4절)에 전용 서브모델을 투입하는 마지막 시도도, 13,000여 개라는 작은 표본으로는 69만 행 전체로 학습한 글로벌 모델을 이기지 못해 실패하였다(0.7353 vs 0.7428). 이는 데이터가 이미 소수의 명확한 규칙으로 생성된 합성 데이터라 원본 피처만으로 트리 모델이 핵심 신호를 충분히 포착했고, 추가 피처·다른 모델·세그먼트 특화 전부 새 정보가 아니라 중복·노이즈로 작용했기 때문으로 판단된다.
...
최종적으로 **EDA → 라벨 규칙 역추적 → 결측 복구 Feature Engineering → 결정규칙 비교 → Hyperparameter Tuning → Stratified 5-Fold CV → Full Training**의 파이프라인을 구축하였으며, 8가지 추가 개선 시도(피처엔지니어링 5가지, 모델 앙상블 1가지, 세그먼트 서브모델 1가지)를 전부 검증하고 기각한 끝에 최종 **Balanced Accuracy 0.94987**을 이 프로젝트의 결과로 확정하였다 (대회 1위 0.95085 대비 약 0.001 차이로, 3피처 기반 이론적 상한에 근접한 수준). 8번의 서로 다른 방향의 실패가 이 수치를 "국소 최적점"이 아니라 "이 데이터와 피처셋이 허용하는 실질적 상한"으로 신뢰할 수 있게 만드는 근거다.
